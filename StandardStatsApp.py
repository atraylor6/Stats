...
def calculateRollingExcessReturns(data, window_size, benchmark_column):
    ...
    for column in data.columns:
        rollingExcessReturns[column] = (
            data[column].rolling(window=window_size).apply(lambda x: annualizedReturn(x)).dropna()
        ) - data[benchmark_column].rolling(window=window_size).apply(lambda x: annualizedReturn(x)).dropna()
...
def excessReturnSkew(excessreturns):
    return excessreturns.apply(skew, nan_policy='omit')
...
def relativeMaxDrawdown(excessreturns):
    relative_dd = {}
    for column in excessreturns.columns:
        cum_rel = (1 + excessreturns[column]).cumprod()
        peak = cum_rel.cummax()
...
def standardStats(df, apiKey, benchmarkColumn):
    ...
    rolling_excess_returns_3yr = calculateRollingExcessReturns(returns, 36, benchmarkColumn)
